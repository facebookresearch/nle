...
import collections
from nle.nethack.actions import USEFUL_ACTIONS
import torch
...
def get_action_space_mask(action_space, reduced_action_space):
    mask = np.array([int(a in nethack.USEFUL_ACTIONS) for a in nethack.ACTIONS])
...
class BaselineNet(NetHackNet):
    """This model combines the encodings of the glyphs, top line message and blstats into
    a single fixed-size representation, which is then passed to an LSTM core before generating
    a policy and value head for use in an IMPALA like architecture.
...
class GlyphEncoder(nn.Module):
    """This glyph encoder first converts the glyphs (integers up to 6000) to a tuple
    of more structured representations. These consist of....

    These structure representations are embedded and concatenated, before encoding with a
    number of CNN layers.  This operation is repeated with a crop of the structured reprentations
    taken around the characters position, and the two representations are concatenated before returning.
    """
    ...
    def __init__(self, flags, rows, cols, crop_dim, device=None):
        ...
        assert (
            K % 8 == 0
        ), "This glyph embedding format needs embedding dim to be a multiple of 8"
        unit = K // 8
...
class MessageEncoder(nn.Module):
    ...
    It works by using a learnt embedding for each character before passing the embeddings
    through 6 CNN layers.
