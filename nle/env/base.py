...
class NLE(gym.Env):
    ...
    Args:
        savedir (str): path to save archives into. Defaults to None.
        use_archivefile (bool): Defaults to True.
        env_id (int): ???. Defaults to 0.
        character (str): name of character. Defaults to "mon-hum-neu-mal".
        max_episode_steps (int): maximum amount of steps allowed before the game
            is forcefully quit. In such cases, ``info["end_status"]`` will be
            equal to ``StepStatus.ABORTED``. Defaults to 5000.
        actions (list): list of available nethack actions. Defaults to ACTIONS.
        observation_keys (list): keys to use when creating the observation.
            Defaults to all.
        nethack_options (list): list of game options to initialize NetHack.
            Defaults to NETHACKOPTIONS.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_archivefile = use_archivefile
        self.env_id = env_id
        self.character = character
        self._max_episode_steps = max_episode_steps

        if actions is None:
            self._actions = DEFAULT_ACTIONS
        else:
        ...
        if use_archivefile:
            ...
            self.archivefile = os.path.join(
                self.savedir, "nethack.%%(pid)i.%%(time)s.%i.zip" % env_id
            )
        else:
            self.savedir = None
            self.archivefile = None

        # Setup stats csv only if archiving is enabled
        self._setup_statsfile = use_archivefile
        if not self._setup_statsfile:
            self._stats_file = None
            self._stats_logger = None

        logger.debug("Starting env with id %i", env_id)

        if "playername" not in kwargs:
            kwargs["playername"] = "Agent%(pid)i-" + self.character
        self.env = nethack.NetHack(archivefile=self.archivefile, **kwargs)

        self._random = random.SystemRandom(env_id)
    ...
    def step(self, action: int):
        ...
        if end_status:
            ...
            info["stats"] = stats._asdict()
            self._stats_logger.writerow(stats._asdict())
    ...
    def _quit_game(self, response, done, info):
        ...
        if not done:
            ...
            logger.error(
                "Warning: smooth quitting of game failed, aborting (archive %s).",
                self.env._archive,
            )
