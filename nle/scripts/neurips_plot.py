...
"""
Script for plotting results from an NLE agent's logs.csv file.
...
```
python -m nle.scripts.plot path/to/run/logs.csv --window 10
```

Plot a specific run to a specific window size. (PATH/logs.csv is found automatically)
```
...
```
python -m nle.scripts.plot path/to/multi_runs --no_legend
```
...
```
python -m nle.scripts.plot path/to/multi_runs/2020-05 --xrange 0,1e8 --yrange='-10,80'
```
"""
...
parser.add_argument(
    ...
    "--file",
    type=str,
    default="~/torchbeast/latest/logs.csv",
    help="file to plot or directory to look for log files",
)
parser.add_argument(
    "-k", "--key", type=str, default="mean_episode_return", help="logged value to plot"
)
parser.add_argument(
...
def plot_single_ascii(
    ...
    df = pd.read_csv(target, sep=",", engine="python")
    steps = np.array(df["step"])

    if window < 0:
        window = len(steps) // width + 1
    window = df[key].rolling(window=window, min_periods=0)
...
def collect_logs(target):
    ...
    """
    dfs = []
    for child in sorted(glob.iglob(str(target))):
        child = Path(child)
        try:
            df = pd.read_csv(child, sep=",")
            # TODO: remove rows with nan? maybe bad as will damage rolling window
            # df[df[key] == df[key]].copy()
            if len(df) > 0:
                name = str(child.parent)
                dfs.append((name, df))
        except pd.errors.EmptyDataError:
            print("Found no data in %s" % str(child))
        except pd.errors.ParserError:
            print("Error reading file %s" % str(child))

    if len(dfs) == 0:
        # didn't find any valid csv logs
...
def plot_multiple_ascii(
    ...
    if window < 0:
        max_size = max(len(df["step"]) for _name, df in dfs)
    ...
    for name, df in dfs:
        steps = np.array(df["step"])
        if window > 1:
            roll = df[key].rolling(window=window, min_periods=0)
            try:
    ...
    if len(dfs) == 1:
        print("Plotting only one found run: %s" % dfs[0][0])
        plot_single_ascii(
            dfs[0][0] + "/logs.csv", width, height, key, window, xrange, yrange
        )
        return
...
def plot(flags):
    ...
    if target.is_file():
        # plot single torchbeast run, path/to/logs.csv
        if target.suffix == ".csv":
            ...
            raise RuntimeError(
                "Filetype not recognised (expected .csv): %s" % target.suffix
            )
    elif (target / "logs.csv").is_file():
        # next check if this is actually a single run directory with file "logs.csv"
        plot_single_ascii(
            target / "logs.csv",
            flags.width,
            flags.height,
