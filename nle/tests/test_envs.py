...
def get_nethack_env_ids():
    ...
    return [spec.id for spec in specs if spec.id.startswith("NetHack")]


def check_rollouts(env0, env1, max_rollout_len):
    """Checks that two environments return the same rollout.
...
class TestGymEnv:
    ...
    def test_rollout(self, env_name, rollout_len=100):
    ...
    def test_seed_interface_output(self, env_name):
    ...
    def test_seed_rollout_from_nethack(self, env_name, rollout_len=100):
        ...
        del obs1["message"]
        np.testing.assert_equal(obs0, obs1)
        check_rollouts(env0, env1, rollout_len)

    def test_seed_rollout_seeded(self, env_name, rollout_len=100):
    ...
    def test_seed_rollout_seeded_int(self, env_name, rollout_len=100):
        """Tests that two seeded envs return same step data."""
        env0 = gym.make(env_name)
        env1 = gym.make(env_name)
    ...
    def test_render_ansi(self, env_name, rollout_len=100):
        env = gym.make(env_name)
        env.reset()
        for _ in range(rollout_len):
