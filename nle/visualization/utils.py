...
def draw_all(glyphs, agent, last_obs):
    ...
    try:
        stats_frame = draw_stats(agent, width, height)
    except:
    ...
    frame = np.concatenate([glyph_frame, stats_frame], axis=0)
...
class Agent:
    ...
    def update_chracters(self, role, alignment, race, gender):
        self.role = self.character.name_to_role[role]
        self.alignment = self.character.name_to_alignment[alignment]
        self.race = self.character.name_to_race[race]
        self.gender = self.character.name_to_gender[gender]


def draw_stats(agent, width, height):
    ch = agent.character
    ...
    #print("agent.race: ", agent.race)
    #print("agent.alignment: ", agent.alignment)
    #print("agent.gender: ", agent.gender)
    #print("ch.name_to_role.items(): ", ch.name_to_role.items())
    #print("ch.name_to_role.items()['Valkyrie']: ", ch.name_to_role.items()['Valkyrie'])

    # general character info
    txt = [
        {v: k for k, v in ch.name_to_role.items()}[agent.role],
        {v: k for k, v in ch.name_to_race.items()}[agent.race],
        {v: k for k, v in ch.name_to_alignment.items()}[agent.alignment],
        {v: k for k, v in ch.name_to_gender.items()}[agent.gender],
    ]
