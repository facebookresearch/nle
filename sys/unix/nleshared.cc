...
struct Mmap {
    ...
    static Mmap
    make(const Fd &fd, size_t offset, size_t size, bool write, bool shared)
    {
        errno = 0;
        void *ptr =
            ::mmap(nullptr, size, write ? PROT_READ | PROT_WRITE : PROT_READ,
                   shared ? MAP_SHARED : MAP_PRIVATE, (int) fd, offset);
    ...
    static Mmap
    shared(const Fd &fd, size_t offset, size_t size, bool write)
    {
        return make(fd, offset, size, write, true);
    }
    static Mmap
    private_(const Fd &fd, size_t offset, size_t size, bool write)
    {
...
class NHLoader
{
    // The raw file data is kept around as ELF headers are generally read from
    // it, and there is no guarantee that the in-memory layout mirrors the
    // file data.
    Mmap fileMem;
    Elf64_Ehdr *hdr;
    // The "base" form of the library, mapped into memory with all symbols
    // resolved.
    Fd sharedFd;
    Image sharedImage;
    ...
    void
    load(std::string libPath,
         const std::unordered_map<std::string, void *> &overrides)
    {
        ...
        forPh([&](Elf64_Phdr *ph) {
            if (ph->p_type == PT_LOAD) {
                ...
                endaddr =
                    std::max(endaddr, (size_t)(ph->p_vaddr + ph->p_memsz));
            }
        });
        ...
        // Copy over all initialized data from the file to the base shared
        // image.
        forPh([&](Elf64_Phdr *ph) {
            if (ph->p_type == PT_LOAD) {
                std::memcpy(sharedImage.base + ph->p_vaddr,
                            fileMem.data() + ph->p_offset, ph->p_filesz);
            }
        });
        // And link it.
        link(overrides);
        // We should never write to the shared image again, so make it
        // read-only
        mprotect(sharedImage.mem.data(), sharedImage.mem.size(), PROT_READ);
    }

    void
    boundsCheck(std::byte *begin, std::byte *end)
    {
        if (begin < fileMem.data() || end > fileMem.data() + fileMem.size()) {
            throw std::runtime_error(
                "Out of bounds access parsing ELF (corrupt file?)");
        }
    }
    ...
    // Dynamic linking. Loads required libraries, resolves symbols and
    // performs relocations. Some relocations must also be performed at fork
    // time.
    void
    link(const std::unordered_map<std::string, void *> &overrides)
    {
        ...
        forPh([&](Elf64_Phdr *ph) {
            if (ph->p_type == PT_DYNAMIC) {
                Elf64_Dyn *dyn =
                    (Elf64_Dyn *) (fileMem.data() + ph->p_offset);
                Elf64_Dyn *dynEnd =
                    (Elf64_Dyn *) (fileMem.data() + ph->p_offset
                                   + ph->p_filesz);
                while (dyn < dynEnd) {
        ...
        auto copy64 = [&](std::byte *dst, Address addr) {
            if (addr.isRelative) {
                forkRelocations.push_back(
                    { uint64_t(dst - base), addr.value });
            }
            std::byte *value = addr.isRelative ? base + addr.value
                                               : (std::byte *) addr.value;
                ...
                while (dyn < dynEnd) {
                    ...
                    case DT_RELA: {
                        if (relasz > 0 && relaent > 0) {
                            for (size_t i = 0; i != relasz / relaent; ++i) {
                                Elf64_Rela *rela =
                                    (Elf64_Rela *) (base + dyn->d_un.d_ptr
                                                    + relaent * i);
                                doRela(rela);
                            }
                        }
                    ...
                    case DT_PLTREL: {
                        if (dyn->d_un.d_val != DT_RELA) {
                            throw std::runtime_error(
                                "Unsupported value for DT_PLTREL");
                        }
                        break;
                    }
                    case DT_JMPREL: {
  ...
  public:
    ...
    template <typename R, typename... Args> struct Symbol<R(Args...)> {
        size_t offset;

        template <typename Instance>
        R
        operator()(Instance &i, Args... args)
        {
    ...
    struct Instance {
        ...
        // Does not call finalization functions or clean up any other
        // resources
        void
        reset()
        {
        ...
        init()
        {
            ...
            if (loader->initarray) {
                for (size_t i = 0; i != loader->initarraysz / sizeof(void *);
                     ++i) {
                    ((void (*)())(
                        (void **) (image.base + loader->initarray))[i])();
                }
            }
        }
        ...
        {
            if (loader->finiarray) {
                for (size_t i = 0; i != loader->finiarraysz / sizeof(void *);
                     ++i) {
                    ((void (*)())(
                        (void **) (image.base + loader->finiarray))[i])();
                }
            }
            if (loader->finifunc) {
        ...
        auto
        call(Symbol<Sig> func, Args &&... args)
        {
            return ((Sig *) (void *) (image.base + func.offset))(
    ...
    symbol(std::string_view name)
    {
        ...
        for (size_t i = 0; i != n; ++i) {
            ...
            if (shdr->sh_type == SHT_SYMTAB) {
                size_t strtab =
                    ((Elf64_Shdr *) (fileMem.data() + hdr->e_shoff
                ...
                for (auto i = begin; i < end; i += shdr->sh_entsize) {
                    ...
                    std::string_view sname =
                        (const char *) (fileMem.data() + strtab
                                        + sym->st_name);
                    if (sname == name) {
    ...
    fork()
    {
        ...
        std::byte *base = (std::byte *) mem.data()
                          + (sharedImage.base - sharedImage.mem.data());
    ...
    reset(Instance &i)
    {
        ...
        forPh([&](Elf64_Phdr *ph) {
            if (ph->p_type == PT_LOAD && (ph->p_flags & PF_W)) {
                std::memcpy(i.image.base + ph->p_vaddr,
                            sharedImage.base + ph->p_vaddr, ph->p_memsz);
            }
        });
...
init(Env *env)
{
    ...
    env->nethack_exit =
        env->instance.loader->symbol<void(int)>("nethack_exit");
}
...
const std::unordered_set<std::string_view> allowedFiles = { "/dev/urandom" };

// List of files in nethackdir that we allow read-only access to
const std::unordered_set<std::string_view> nethackdirFiles = { "sysconf",
                                                               "nhdat" };

int
creat(const char *path, int)
{
...
extern "C" void *
nleshared_open(const char *dlpath)
{
    return new env::Env{ env::getLoader(dlpath).fork() };
}
extern "C" void
nleshared_close(void *handle)
{
    env::Env *env = (env::Env *) handle;
    delete env;
}
extern "C" void
nleshared_reset(void *handle)
{
    env::Env *env = (env::Env *) handle;
    env->reset();
}
extern "C" void *
nleshared_sym(void *handle, const char *symname)
{
    env::Env *env = (env::Env *) handle;
    return (void *) env->instance.loader->symbol<void()>(symname).resolve(
        env->instance);
}
extern "C" void
nleshared_set_current(void *handle)
{
