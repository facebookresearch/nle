...
#define TILE_FILE "tile.py"

#ifdef AMIGA
#define SOURCE_TEMPLATE "NH:nle/tiles/%s"
#else
#ifdef MAC
#define SOURCE_TEMPLATE ":nle:tiles:%s"
#else
#define SOURCE_TEMPLATE "../../nle/tiles/%s"
#endif
#endif
...
main()
{
    ...
    // process_substitutions(ofp); Not processing substitutions ATM

    Fprintf(ofp, "\nMAXMONTILE = %d\n", lastmontile);
    Fprintf(ofp, "MAXOBJTILE = %d\n", lastobjtile);
    Fprintf(ofp, "MAXOTHTILE = %d\n", lastothtile);
#ifdef STATUES_LOOK_LIKE_MONSTERS
    Fprintf(ofp, "/* MAXSTATUETILE = %d */\n", laststatuetile);
#endif
    Fprintf(ofp, "\n# tile.py\n");
